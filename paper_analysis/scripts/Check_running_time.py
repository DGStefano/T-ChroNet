...
if __name__ == '__main__':
    ...
    for process_num in [ 2 , 5 , 15 , 20 , 25, 30 , 35 ,40] :
        print(f'number of processors {process_num}')
        n_processes = process_num
        estimated_time = []
        for nodes_number in [1000 , 5000 , 10000 , 20000 , 40000 , 60000 ]  : #
            ...
            data_matrix_df = data_matrix_df.sample(n = nodes_number )
        ...
        #    X = np.load('/mnt/nas-safu/analysis/PhDsdigiove/method_coAcces/data/BALL/ClusteredPatients_Ball_Multicov.npy', mmap_mode='r')
            ...
            estimated_time.append(time.time() - startTime)
            
            shutil.rmtree(f'{tmp_dir}')
            print("deleated")
            print(f'estimated times {estimated_time}')
        time_out_part = pd.DataFrame({'index': range(6) , 'row_name': [1000 , 5000 , 10000 , 20000 , 40000 , 60000 ] , 'estimated_time' : estimated_time , "n_processors" :  [n_processes , n_processes , n_processes , n_processes , n_processes , n_processes] })
