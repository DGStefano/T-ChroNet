...
series_thp1 <- build_TChroNetSeries_object("~/T-ChroNet/paper_analysis/data/THP1/th",matrix_path ,method = "Leiden",resolutions_list = seq(0.5, 1.5, 0.1),run_cd = T,seed = 1234 , transitivity = FALSE)

##### FINDING THE BEST THRESHOLD
series_thp1@best_th <- find_best_th(series_thp1)

##### PLOTTING THE RAND INDEX MATRIX OF NEIGHBOURS THRESHOLD AND RESOLUTIONS
pdf("~T-ChroNet/paper_analysis/data/THP1rand_index_map.pdf" ,
  height = 10, 
  width = 8,
  family = "ArialMT",
  useDingbats = FALSE)
...
pdf("~T-ChroNet/paper_analysis/data/THP1sankey_plot_res_1_3.pdf" ,
  height = 5, 
  width = 9,
  family = "ArialMT",
  useDingbats = FALSE)
...
pdf("~T-ChroNet/paper_analysis/data/THP1relative_lcc.pdf" ,
  height = 5, 
  width = 9,
  family = "ArialMT",
  useDingbats = FALSE)
...
to_plot <- plot_modularity(thp1_th08)+
  theme(text = element_text(size = 15))
pdf("~T-ChroNet/paper_analysis/data/THP1modularity_th08.pdf" ,
  height = 5, 
  width = 9,
  family = "ArialMT",
  useDingbats = FALSE)
...
pdf("~T-ChroNet/paper_analysis/data/THP1community_sankey_th08.pdf" ,
  height = 5, 
  width = 9,
  family = "ArialMT",
  useDingbats = FALSE)
...
ggplot(aes(x = clusters_1 , y = peaks)) +
  ...
  ylab("Size")+
  theme(text = element_text(size = 15))
pdf("~T-ChroNet/paper_analysis/data/THP1community_size.pdf" ,
  height = 5, 
  width = 9,
  family = "ArialMT",
  useDingbats = FALSE)
...
to_plot <- plot_stacked_annotation(thp1_th08) +
  ...
  ylab("") +
  xlab("")
pdf("~T-ChroNet/paper_analysis/data/THP1stachek_annotations_th08_res1_0.pdf" ,
  height = 5, 
  width = 9,
  family = "ArialMT",
  useDingbats = FALSE)
...
for (x in seq_along(list_communities_09)) {
  ...
  out_file <- paste("~/T-ChroNet/paper_analysis/data/THP1/communities/community_hg38_" , as.character(x) , ".bed" , sep ="")
...
pdf("~T-ChroNet/paper_analysis/data/THP1boxplot_trends.pdf" ,
  height = 9, 
  width = 5,
  family = "ArialMT",
  useDingbats = FALSE)
...
pdf("~T-ChroNet/paper_analysis/data/THP1lineplot_trends.pdf" ,
  height = 9, 
  width = 5,
  family = "ArialMT",
  useDingbats = FALSE)
...
to_plot <- ggplot(final_df_selected_Columns , aes(x = community , y = Description , color = qval_log10 , size = FoldEnrichment)) +
  ...
  theme(text = element_text(size = 15))
pdf("~T-ChroNet/paper_analysis/data/THP1Hallmarks.pdf",
  height = 5, 
  width = 9,
  family = "ArialMT",
  useDingbats = FALSE)
...
to_plot <- plot_cistrome("/home/sdigiove/T-ChroNet/paper_analysis/data/THP1/cistrome/" , tf_name_file = "~/T-ChroNet/paper_analysis/TFs_screening/TF_names_v_1.01.txt", ) +
  theme(text = element_text(size = 15))
pdf( "~T-ChroNet/paper_analysis/data/THP1TFs.pdf",
  height = 8, 
  width = 7,
  family = "ArialMT",
  useDingbats = FALSE)
...
pdf("~T-ChroNet/paper_analysis/data/THP1heatmap.pdf",
  height = 9, 
  width = 5,
  family = "ArialMT",
  useDingbats = FALSE)
...
pdf("~T-ChroNet/paper_analysis/data/THP1rna_genes.pdf" ,
  height = 8, 
  width = 7,
  family = "ArialMT",
  useDingbats = FALSE)
...
pdf("~/T-ChroNet/paper_analysis/data/THP1/overlaps_communities.pdf"  ,
  height = 6, 
  width = 6,
  family = "ArialMT",
  useDingbats = FALSE)
