...
plot_sankey_fixed_resolution(series_thp1 , resolution = 1.5)
# ggsave("/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/thp1/pictures/sankey_plot_res_1_5.png" , height = 5 , width = 9 , units = 'in', dpi = 300)
...
plot_cistrom("/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/review_results/thp1/bed/cistromdb/" , tf_name_file = "/mnt/nas-safu01/analysis/scripts/ScriptSdigiove/RegNetATACProject/T-ChroNet/paper_analysis/TFs_screening/TF_names_v_1.01.txt", ) +
  theme(text = element_text(size = 15))
ggsave("/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/thp1/pictures/TFs_Cistrome.png", height = 5 , width = 9 , units = 'in', dpi = 300)

### Liver
series_liver <- read_rds("/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/liver/series_obj_liver.rds")

plot_randindex_map(series_liver ,"/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/liver/picture/rand_index_map.png" )
plot_sankey_fixed_resolution(series_liver , resolution = 1.1)
ggsave("/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/liver/picture/sankey_plot_res_1.1.png" , height = 5 , width = 9 , units = 'in', dpi = 300)
...
ggsave("/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/liver/picture/community_sankey_th08.png", height = 5 , width = 9 , units = 'in', dpi = 300)

liver_obj <- find_best_resolution(liver_obj)

liver_obj <- annotate_regions_from_bed(liver_obj , bed_path = "../toy_data/data/mm10-cCREs_mod.bed" , genome = 'mm10')
plot_stacked_annotation(liver_obj , resolution = 1.1)
# ggsave("/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/liver/picture/stachek_annotations_th08_res1.1.png", height = 5 , width = 9 , units = 'in', dpi = 300)
...
### Save communities hg38
for (x in seq_along(list_communities_15)) {
  comm <- list_communities_15[[x]]
  comm_hg38 <- thp1_th08@lifted_coords |> filter(node %in% comm ) |> select(chr_hg38,start_hg38,end_hg38) 
  out_file <- paste("/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/review_results/thp1/bed/community_hg38_" , as.character(x) , ".bed" , sep ="")
  write_delim(comm_hg38 , out_file , delim ="\t" , col_names = F)
}
...
# THP-1
folders_to_compare = list.files('/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/review_results/thp1/bed/cistromdb/')

for (community_num in seq_along(folders_to_compare) ){ 
    
    community <- folders_to_compare[[community_num]]
    to_read = paste('/mnt/nas-safu02/sdigiove_workspace/check_th_TCHRONET/review_results/thp1/bed/cistromdb/' ,community , sep = "")
...
tfs_name = read_delim("/mnt/nas-safu01/analysis/scripts/ScriptSdigiove/RegNetATACProject/T-ChroNet/paper_analysis/TFs_screening/TF_names_v_1.01.txt" , delim = "\t" , col_names = F)
tfs_name <- tfs_name %>%
...
# final_homer_scatterplot |> pull(Factor) |> unique() |> length()
